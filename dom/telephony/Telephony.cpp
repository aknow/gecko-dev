...
public:
  ...
  NotifyDialError(const nsAString& aError) {
    mTelephony->RejectPromiseByString(mPromise, aError);
    return NS_OK;
  }
...
Telephony::RejectPromiseByString(Promise* aPromise, const nsAString& aString) {
  ...
  JSString* strValue = JS_NewUCStringCopyZ(cx, nsString(aString).get());
  JS::Rooted<JS::Value> result(cx, STRING_TO_JSVAL(strValue));
  aPromise->MaybeReject(cx, result);
}
                        ...
                        bool aIsEmergency)
{
  ...
  if (!IsValidNumber(aNumber) || !IsValidServiceId(aServiceId)) {
    RejectPromiseByString(promise, NS_LITERAL_STRING("InvalidAccessError"));
